...
for gdp in namelist:
        for bbp in coupling:
                ...
                if first_set:
                        Z_jets_full = np.concatenate((Z_jets_full,Z_jets),axis=0)
                        Z_mu_full = np.concatenate((Z_mu_full,Z_mu),axis=0)
                        Z_el_full = np.concatenate((Z_el_full,Z_el),axis=0)
                        Z_flat_full = np.concatenate((Z_flat_full,Z_flat),axis=0)
                        Z_Y_full = np.concatenate((Z_Y_full,Z_Y),axis=0)
                else:
                        Z_jets_full = Z_jets
                        Z_mu_full = Z_mu
                        Z_el_full = Z_el
