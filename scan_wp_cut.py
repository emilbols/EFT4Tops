...
xsec_error = [0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01] # pb
n=0
...
for wp_cut in wp_list:
            for sample in samples:
						...
						sample_size = X_flat.shape[0]
                                                ...
                                                events_cut.append(x_sec[sample][n]*X_flat[[X_flat[:,1] > wp_cut]].shape[0]/sample_size)
                                                ...
                                                uncs['xsec_cut'] = sqrt( (sqrt(lum*x_sec[sample][n]*X_flat[X_flat[:,1] > wp_cut].shape[0]/sample_size)/lum)**2 + (frac_syst*x_sec[sample][n]*X_flat[X_flat[:,1] > wp_cut].shape[0]/sample_size)**2)
                                                SM['xsec'] = x_sec[sample][n]
                                                SM['xsec_cut'] = x_sec[sample][n]*X_flat[X_flat[:,1] > wp_cut].shape[0]/sample_size
                                                n+=1
                        ...
                        for name_sec,xsec in objects.items():
                                    err = np.multiply(xsec_error,xsec)
                                    ...
                                    test_y = coeff[0]*(1 + coeff[1]*coupling_strengths + coeff[2]*coupling_strengths*coupling_strengths)
                                    chi2 = sum(sqrt((test_y-xsec)**2/(xsec_error)**2))
